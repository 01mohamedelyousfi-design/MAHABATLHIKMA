...
| # | Action | Status | Notes |
|---|---|---|---|
| 3.1 | Move the 3.66 MB `value-intro.mp4` to YouTube (unlisted embed) or Cloudflare Stream | ⏸ **deferred by owner decision (2026-09-23) — keeping local autoplay for now.** When revisited: upload to the site's YouTube channel, then swap the `<video>` tag for a nocookie iframe (pattern ready in `assets/js/howto-video.js`). `identity-reel.mp4` no longer needs this: it was unreferenced and deleted with the duplicate tree in Phase 2.1 | Faster deploys, less bandwidth |
